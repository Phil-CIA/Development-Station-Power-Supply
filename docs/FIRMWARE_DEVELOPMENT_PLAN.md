...
| Fault/status reporting (OVP/OCP/OTP, output-enable, CV/CC) | ❌ Hardcoded placeholder | `stm32-bluepill-bringup/src/main.cpp:963-964` | `status = 0xF0` and `protection_flags = 0x00` are constants — always "everything on, no faults." Also note pin-contract mismatch: firmware assigns `PIN_FAULT_CRITICAL_SUM = PA3` but current Rev-C netlist routes PA3 on `RXD_ESP`; see `docs/STM32_BLUEPILL_PIN_TABLE.md`. |
