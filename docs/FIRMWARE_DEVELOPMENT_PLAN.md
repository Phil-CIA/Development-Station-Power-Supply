...
| Per-rail calibration + persistent config storage | ⚠️ Reference only, not ported | `src/rev1/main.cpp` (`Preferences`/NVS) | STM32 has no persistent storage yet. `stm32-bluepill-bringup` already has a working bring-up test for the W25Q128 external SPI flash — natural storage target. |
...
| Display → host command channel (`CMD:`/`ACK:`/`ERR:`/`EVT:`) | ❌ Not implemented, either end | — | Specified in `docs/DISPLAY_INTERFACE_STANDARD.md`. Telemetry today only flows STM32 → display; there is no path for a button press on the display to change output enable, current limit, or thresholds on the HAT. |
