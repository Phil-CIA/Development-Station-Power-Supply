...
| Fault/status reporting (OVP/OCP/OTP, output-enable, CV/CC) | ⚠️ Runtime-derived, routing-limited | `stm32-bluepill-bringup/src/main.cpp` (telemetry publish path) | `status` and `protection_flags` are now computed from live state (channel enable/CV-CC, OVP thresholds, thermal warn/OTP). Rev-C limitation remains: `FAULT_CRITICAL_SUM` is not routed to an STM32 GPIO (`PIN_FAULT_CRITICAL_SUM = -1`), so hardware OCP trip reporting is unavailable on this revision until a fault-input net is routed. |
...
- `firmware/stm32-fault-status-bits` — in progress: `status`/`protection_flags` now use real runtime values (output-enable, CV/CC, OVP, thermal warn/OTP). Remaining gap is true hardware OCP reporting because current Rev-C netlist does not route `FAULT_CRITICAL_SUM` to STM32 GPIO.
